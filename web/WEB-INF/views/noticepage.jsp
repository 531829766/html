...
<form action="${pageContext.request.contextPath}/noticepage" method="get">
    用户名称：<input type="text" name="title" value="${noticeparams.title}">
    用户余额：<input type="text" name="content" value="${noticeparams.content}">
    <input type="submit" value="搜索">
</form>
<form action="${pageContext.request.contextPath}/noticedelete" method="post">
    <table>
        <tr>
            <th>&nbsp;</th>
            <th>标题</th>
        ...
        <c:forEach items="${noticepageData.list}" var="cn">
            <tr>
                <td><input type="checkbox" name="ids" value="${cn.id}"/></td>
                <td>${cn.title}</td>
                <td>${cn.content}</td>
                <td><a href="${pageContext.request.contextPath}/noticeedit/${ct.id }">修改</a></td>
            </tr>
        </c:forEach>
        <tr>
            <td class="td2">
...
<form action="${pageContext.request.contextPath}/noticeto_add" method="post">
